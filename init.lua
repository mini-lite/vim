...
local function center_selection_in_view(_, line) -- put selected line in middle of visible view
...
local function deselect(to_start) -- collapse selection
  local doc = get_doc()
  if not doc then
    return
  end
...
local function build_combo(key) -- detects a combo
  ...
  if vim.mod_keys.alt then
    table.insert(mods, "alt")
  end
  if vim.mod_keys.shift then
  ...
  if doc and doc.filename then
    ...
    elseif type == "keypressed" then
      if vim.modifier_keys[a] == nil and (vim.mod_keys.ctrl or vim.mod_keys.alt or vim.mod_keys.shift) then
        ...
        if vim.known_keymaps[cmb] == nil then -- nothing, let key reach original on_event
        else
...
local function flash(l1, c1, l2, c2, color, time, doc)
  core.add_thread(function()
    ...
    core.redraw = true
    deselect(true) -- selection is lost here
  end)
end
...
local translations = {
  ...
  ["start-of-word"] = function(doc, line, col)
    ...
    while true do
      ...
      if prev_line == line and prev_col == col then
        break
      end
      local prev_type = classify(doc:get_char(prev_line, prev_col))
      if prev_type ~= ctype then
        break
      end
  ...
  ["this-char"] = function(doc, l, c, ch, dir) -- find character and translate to it
    ...
    while true do
      ...
      if nl == l and nc == c then
        return
      end
      l, c = nl, nc
      if (doc.lines[l] or ""):sub(c, c) == ch then
        return l, c
      end
    end
  end,
["this-block"] = function(doc, l, c, open_ch, close_ch, dir)
  local depth = 0
  while true do
    local nl, nc = doc:position_offset(l, c, dir)
    ...
    local cur = (doc.lines[l] or ""):sub(c, c)
    if dir > 0 then
      if cur == open_ch then
        depth = depth + 1
      elseif cur == close_ch then
        if depth == 0 then return l, c end
        depth = depth - 1
      end
    else
      if cur == close_ch then
        depth = depth + 1
      elseif cur == open_ch then
        if depth == 0 then return l, c end
        depth = depth - 1
      end
    end
...
vim.motions = {
  ...
  [">"] = function(doc, l, c) -- pass no motion
  end,
...
-- update known combo keymaps
for k in pairs(vim.normal_keys) do
  vim.known_keymaps[k] = true
end
for k in pairs(vim.visual_keys) do
  vim.known_keymaps[k] = true
end
...
find_match = function(doc, line, col)                  -- match anything
  ...
  for l = line, dir == 1 and #doc.lines or 1, dir do
    local start, stop, step =
      l == line and col + dir or (dir == 1 and 1 or #doc.lines[l]), dir == 1 and #doc.lines[l] or 1, dir
...
vim.normal_keys[":"] = function() -- command to launch the command line
...
vim.normal_keys["/"] = function() -- command to launch the search line
